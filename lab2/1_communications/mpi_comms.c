...
void test_asynchronous(int rank, int size)
{
    ...
    for(int i = 0; i < NUM_TEST; i++) {
        ...
        MPI_Isend(&sbuf[i], 1, MPI_INT, send_core, 0, MPI_COMM_WORLD, &request[i * 2]);

        if (rank == i)
            usleep(WAITING_TIME);

        MPI_Irecv(&rbuf[i], 1, MPI_INT, recv_core, 0, MPI_COMM_WORLD, &request[i * 2 + 1]);
    }

    MPI_Waitall(NUM_TEST * 2, request, MPI_STATUSES_IGNORE);
