...
int prime_static ( int n ) {
	...
	#pragma omp parallel for schedule(static,16) private(i, j, prime) reduction(+:total)
	for ( i = 2; i <= n; i++ ){
		prime = 1;
		for ( j = 2; j < i; j++ ){
			if ( i % j == 0 ) {
...
int prime_dynamic ( int n ) {
	...
	#pragma omp parallel for schedule(dynamic, 2) private(i, j, prime) reduction(+:total)
	for ( i = 2; i <= n; i++ ) {
		prime = 1;
		for ( j = 2; j < i; j++ ) {
			if ( i % j == 0 ) {
